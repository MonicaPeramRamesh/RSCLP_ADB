...
Pre-requisites: 
  ...
  - Product master table must exist: rsmas_catalog.rsmas_productmaster_schema.product_master
...
# ------------------------------
@dlt.table(
    name='rsmas_catalog.rsmas_bronze_schema.daily_sales',
...
def parsed_sales_view():
    ...
    bronze_df = dlt.read_stream("rsmas_catalog.rsmas_bronze_schema.daily_sales")
...
# ------------------------------
@dlt.table(
    name="rsmas_catalog.rsmas_silver_schema.daily_sales",
...
# ------------------------------
@dlt.table(
    name='rsmas_catalog.rsmas_gold_schema.daily_sales',
...
def daily_sales_gold():
    ...
    silver_stream = dlt.read_stream("rsmas_catalog.rsmas_silver_schema.daily_sales")
...
# ------------------------------
@dlt.table(
    name='rsmas_catalog.rsmas_invalid_data_schema.invalid_daily_sales',
...
def invalid_daily_sales():
    ...
    bronze_stream = dlt.read_stream("rsmas_catalog.rsmas_bronze_schema.daily_sales")

    product_df = (
        spark.read.table("rsmas_catalog.rsmas_productmaster_schema.product_master").filter(F.col('__END_AT').isNull())
            .select("ProductID")
            .cache()
    )
...
# ------------------------------
@dlt.table(
    name='rsmas_catalog.rsmas_monitor_schema.invalid_sales_summary',
...
def invalid_sales_summary():
    ...
    """
    quarantine_stream = dlt.read_stream("rsmas_catalog.rsmas_invalid_data_schema.invalid_daily_sales") \
...
# ------------------------------
@dlt.table(
    name='rsmas_catalog.rsmas_monitor_schema.daily_pipeline_metrics',
...
def daily_pipeline_metrics():
    ...
    """
    # VALID RECORDS
    silver = (dlt.read_stream("rsmas_catalog.rsmas_silver_schema.daily_sales")
                .withWatermark("LoadTimestamp", "1 hour")
    ...
    invalid = (dlt.read_stream("rsmas_catalog.rsmas_invalid_data_schema.invalid_daily_sales")
                  .withWatermark("LoadTimestamp", "1 hour")
